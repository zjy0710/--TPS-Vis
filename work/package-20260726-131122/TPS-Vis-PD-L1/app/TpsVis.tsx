...
function enqueueTileLoad(run: () => Promise<void>) {
...
function waitForImage(url: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    probe.onerror = () => reject(new Error("Patch image decode failed"));
    probe.src = url;
  });
}
...
function HighResolutionPatchTile({
  ...
  useEffect(() => {
    let disposed = false;
    setReady(false);
    const cancel = enqueueTileLoad(async () => {
                    ...
                    {visiblePyramidPatches.map((patch) => (
                      <HighResolutionPatchTile
                        ...
                        left={
                          stageSize.width * 0.5 +
                          zoom *
                            (patch.x / slideWidth - 0.5) *
                            stageSize.width *
                            0.92
                        }
                        top={
                          stageSize.height * 0.485 +
                          zoom *
                            (patch.y / slideHeight - 0.5) *
                            stageSize.height *
                            0.85
                        }
                        width={
