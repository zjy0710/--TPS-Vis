...
export function TpsVis() {
  ...
  const selectedPatch =
    ...
    casePatches[0] ??
    EMPTY_PATCH;
  const visiblePyramidPatches = useMemo(() => {
    if (
      !hasData ||
      zoom < 1.8 ||
      stageSize.width <= 0 ||
    ...
    const layerWidth = stageSize.width * 0.92;
    const layerHeight = stageSize.height * 0.85;
    const centerX = 0.5 - pan.x / (layerWidth * zoom);
    const centerY = 0.5 - pan.y / (layerHeight * zoom);
    const halfWidth =
      stageSize.width / (2 * layerWidth * zoom) +
      Math.max(512 / slideWidth, 0.025);
    const halfHeight =
      stageSize.height / (2 * layerHeight * zoom) +
      Math.max(512 / slideHeight, 0.04);

    return casePatches
      .filter((patch) => {
        const left = patch.x / slideWidth;
        const right = (patch.x + 512) / slideWidth;
        const top = patch.y / slideHeight;
        const bottom = (patch.y + 512) / slideHeight;
        return (
      ...
      .sort((a, b) => {
        const aDistance =
          Math.abs((a.x + 256) / slideWidth - centerX) +
          Math.abs((a.y + 256) / slideHeight - centerY);
        const bDistance =
          Math.abs((b.x + 256) / slideWidth - centerX) +
          Math.abs((b.y + 256) / slideHeight - centerY);
  ...
  const flyToPatch = (patch: PatchItem) => {
    ...
    const layerWidth = layer?.offsetWidth ?? stageWidth * 0.92;
    const layerHeight = layer?.offsetHeight ?? stageHeight * 0.85;
    const layerCenterX =
      (layer?.offsetLeft ?? stageWidth * 0.04) + layerWidth / 2;
    const layerCenterY =
      (layer?.offsetTop ?? stageHeight * 0.06) + layerHeight / 2;
  ...
  const renderLivePan = (nextPan: { x: number; y: number }) => {
    ...
    panFrameRef.current = window.requestAnimationFrame(() => {
      ...
      layer.style.transform = `translate3d(${currentPan.x}px, ${currentPan.y}px, 0) scale(${zoom})`;
    });
  };
  ...
  const commitLivePan = () => {
    ...
    if (imageLayerRef.current) {
      imageLayerRef.current.style.transform = `translate3d(${nextPan.x}px, ${nextPan.y}px, 0) scale(${zoom})`;
  ...
  const onWheel = (event: ReactWheelEvent<HTMLDivElement>) => {
    ...
    const layer = imageLayerRef.current;
    const layerWidth = layer?.offsetWidth ?? stageRect.width * 0.92;
    const layerHeight = layer?.offsetHeight ?? stageRect.height * 0.85;
    const originX =
      (layer?.offsetLeft ?? stageRect.width * 0.04) + layerWidth / 2;
    const originY =
      (layer?.offsetTop ?? stageRect.height * 0.06) + layerHeight / 2;
            ...
            >
              ...
              >
                <img
                  ...
                  draggable={false}
                />
                {visiblePyramidPatches.length ? (
                  <div
                    className="wsi-pyramid-layer"
                    aria-label="当前视野的原始分辨率 Patch 图像层"
                  >
                    {visiblePyramidPatches.map((patch) => (
                      <img
                        key={patch.id}
                        ...
                        decoding="async"
                        style={{
                          left: `${(patch.x / slideWidth) * 100}%`,
                          top: `${(patch.y / slideHeight) * 100}%`,
                          width: `${(512 / slideWidth) * 100}%`,
                          height: `${(512 / slideHeight) * 100}%`,
                        }}
                      />
                    ))}
                  </div>
