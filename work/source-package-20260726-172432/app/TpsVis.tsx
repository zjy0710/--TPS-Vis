...
const MIN_PYRAMID_LEVEL = 0;
const MAX_PYRAMID_LEVEL = 1;
const DETAIL_TILE_ZOOM = 1.05;
...
function levelForZoom(zoom: number) {
  const requestedLevel =
    zoom < DETAIL_TILE_ZOOM
      ? MIN_PYRAMID_LEVEL
      : Math.max(1, Math.ceil(Math.log2(zoom)));
  return clampLevel(
    requestedLevel,
    MIN_PYRAMID_LEVEL,
    MAX_PYRAMID_LEVEL,
...
function worldRectToScreen(
  ...
  frame: ReturnType<typeof fitSlideFrame>,
  zoom: number,
) {
  return {
    left:
      frame.centerX +
      zoom * (rect.x / slide.width - 0.5) * frame.width,
    top:
      frame.centerY +
      zoom * (rect.y / slide.height - 0.5) * frame.height,
    width: zoom * (rect.width / slide.width) * frame.width,
    height: zoom * (rect.height / slide.height) * frame.height,
  };
}
  ...
  for (let row = minRow; row <= maxRow; row += 1) {
    ...
    for (let column = minColumn; column <= maxColumn; column += 1) {
      ...
      visible.push({
        ...
        pixelWidth: Math.max(
          0,
          Math.min(patch.pixelWidth, slideWidth - x),
        ),
        pixelHeight: Math.max(
          0,
          Math.min(patch.pixelHeight, slideHeight - y),
        ),
      });
    }
  }
...
async function decodePatchFile(file: File) {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    image.onerror = () => reject(new Error("Patch image decode failed"));
    image.src = url;
  });
  return {
...
async function buildPatchOverview(
  patches: PatchItem[],
  slideWidth: number,
  slideHeight: number,
) {
  const maxEdge = 2048;
  const scale = Math.min(1, maxEdge / Math.max(slideWidth, slideHeight));
  const canvas = document.createElement("canvas");
  canvas.width = Math.max(1, Math.ceil(slideWidth * scale));
  canvas.height = Math.max(1, Math.ceil(slideHeight * scale));
  ...
  let nextPatch = 0;
  const drawWorker = async () => {
    while (nextPatch < patches.length) {
      const patch = patches[nextPatch];
      nextPatch += 1;
      if (!patch.imageFile) continue;
      try {
        const decoded = await decodePatchFile(patch.imageFile);
        context.drawImage(
          decoded.source,
          0,
          0,
          ...
          patch.x * scale,
          patch.y * scale,
          patch.pixelWidth * scale,
          patch.pixelHeight * scale,
        );
        decoded.close();
      } catch {
        // A missing overview tile must not block the remaining local case.
      }
    }
  };
  await Promise.all(
    Array.from(
      { length: Math.min(6, Math.max(1, patches.length)) },
      drawWorker,
    ),
  );
...
async function buildLocalCase(files: FileList): Promise<CaseItem> {
  ...
  const maxPatchX = Math.max(...patches.map((patch) => patch.x));
  const maxPatchY = Math.max(...patches.map((patch) => patch.y));
  await Promise.all(
    patches
      .filter((patch) => patch.x === maxPatchX || patch.y === maxPatchY)
      .map(async (patch) => {
        if (!patch.imageFile) return;
        const dimensions = await readImageDimensions(patch.imageFile);
        if (!dimensions) return;
        patch.pixelWidth = Math.max(1, Math.min(512, dimensions.width));
        patch.pixelHeight = Math.max(1, Math.min(512, dimensions.height));
      }),
  );
  ...
  const exactSlideWidth = Math.max(
    ...patches.map((patch) => patch.x + patch.pixelWidth),
  );
  const exactSlideHeight = Math.max(
    ...patches.map((patch) => patch.y + patch.pixelHeight),
  );
  const providedOverviewDimensions = await readImageDimensions(thumbnailFile);
  const slideAspect = exactSlideWidth / exactSlideHeight;
  const overviewAspect = providedOverviewDimensions
    ? providedOverviewDimensions.width / providedOverviewDimensions.height
    : 0;
  const overviewAspectError = overviewAspect
    ? Math.abs(overviewAspect / slideAspect - 1)
    : Number.POSITIVE_INFINITY;
  const summaryMatchesFolder =
    !summary.wsi_id || summary.wsi_id.toLowerCase() === rootFolder.toLowerCase();
  const useProvidedOverview =
    summaryMatchesFolder && overviewAspectError <= 0.12;
  const overviewUrl = useProvidedOverview
    ? URL.createObjectURL(thumbnailFile)
    : (await buildPatchOverview(
        patches,
        exactSlideWidth,
        exactSlideHeight,
      )) ?? URL.createObjectURL(thumbnailFile);
...
export function TpsVis() {
  ...
  const selectedPatch =
    ...
    casePatches[0] ??
    EMPTY_PATCH;
  const activePyramidLevel = levelForZoom(zoom);
  const selectedPatchScreenRect = worldRectToScreen(
    {
      x: selectedPatch.x,
      y: selectedPatch.y,
      width: selectedPatch.pixelWidth,
      height: selectedPatch.pixelHeight,
    },
    { width: slideWidth, height: slideHeight },
    slideFrame,
    zoom,
  );
  const stageStatusText = !hasData
    ? "NO CASE LOADED"
    : roiTool
      ? `Level ${activePyramidLevel}/${MAX_PYRAMID_LEVEL} · ${Math.round(
          zoom * 100,
      ...
      : `Level ${activePyramidLevel}/${MAX_PYRAMID_LEVEL} · ${Math.round(
          ...
          .toString()
          .padStart(2, "0")} · (${selectedPatch.x}, ${selectedPatch.y})`;
  ...
  const flyToPatch = (patch: PatchItem) => {
    ...
    const patchCenterX = (patch.x + patch.pixelWidth / 2) / slideWidth;
    const patchCenterY = (patch.y + patch.pixelHeight / 2) / slideHeight;
    const nextPan = {
      x:
        stageWidth / 2 -
        layerCenterX -
        ...
        <section className="center-column">
          <section className="panel wsi-panel">
            ...
            >
                ...
                >
                  ...
                  >
                    {visiblePyramidTiles.map((tile) => {
                      const tileRect = worldRectToScreen(
                        {
                          x: tile.patch.x,
                          y: tile.patch.y,
                      ...
                      return (
                        <HighResolutionPatchTile
                          ...
                          backgroundWidth={
                            zoom *
                            (tile.patch.pixelWidth / slideWidth) *
                            slideFrame.width
                          }
                          backgroundHeight={
                            zoom *
                            (tile.patch.pixelHeight / slideHeight) *
                            slideFrame.height
                          }
                        />
                      );
              ...
              <div className="wsi-selection-viewport">
                ...
                >
                  <span
                    ...
                    style={{
                      left: `${selectedPatchScreenRect.left}px`,
                      top: `${selectedPatchScreenRect.top}px`,
                      width: `${selectedPatchScreenRect.width}px`,
                      height: `${selectedPatchScreenRect.height}px`,
                    }}
                  />
                </div>
              </div>
              ...
              <div className="stage-status" title={stageStatusText}>
                {stageStatusText}
              </div>
            </div>
          </section>
