...
└─ patches/
   └─ <patch_id>/
       ├─ image.jpg
       ├─ cells.csv                  # 推荐；用于动态生成 E-1 / E-2
       ├─ cells_overlay.png          # 可选；存在时优先作为 E-1
       └─ heatmap_overlay.png         # 可选；存在时优先作为 E-2
```

ZIP 压缩包仅用于传递示例数据，网页实际选择的是解压后的文件夹。

当病例只包含 `image.jpg` 和 `cells.csv` 时，网页会读取 `x`、`y`、
`center_prob`、`cell_pos_prob` 与 `cell_pred` 字段，在浏览器端叠加生成
E-1 细胞表型标注和 E-2 概率图；不需要预先导出两张 overlay PNG。

全片主视图在低倍下使用浏览器可直接解码的 `thumbnail.png`；放大到 105%
以上时，网页会按当前视野加载 `patches/` 中的原始图像瓦片，形成
...
全片底图、Patch 瓦片、热力图和选中框统一使用切片原始宽高比进行等比例映射，
不再随 C 模块显示区域的形状横向或纵向拉伸。

如果 `thumbnail.png` 的宽高比或 `wsi_summary.json` 病例编号与当前 Patch
清单不一致，网页会在本地用 Patch 自动生成匹配的低倍概览，避免旧底图与
选中框错位。切片边界按边缘 Patch 的真实像素尺寸计算，partial 瓦片从左上角
按实际范围裁剪。
